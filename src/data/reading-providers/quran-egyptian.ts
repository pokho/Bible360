...
export class QuranEgyptianProvider {
	async loadReadingPlan(): Promise<ReadingPlan> {
		...
		return {
			...
			dailyReadings,
			metadata: {
				title: 'Quran — Egyptian Standard Chronological Order',
				description: 'Surah-level chronological ordering following the 1924 Egyptian Standard edition. Meccan surahs first, then Medinan.',
	...
	private generateDailyReadings(): DailyReading[] {
		const sorted = [...quranSurahData].sort(
			(a, b) => a.egyptianChronologicalOrder - b.egyptianChronologicalOrder
		);
